...
public class MuseIO : MonoBehaviour {
	...
	void Start() {
		...
		Debug.Log ("MUSEIO CLASS INSTANCE WORKING...");
		...
		oscIn.Map("/muse/elements/experimental/concentration", OnConcentration);
	...
	public void OnConcentration(OscMessage message) {
		//ParseFloat (Score[4], message);
		//Test(message);
		//ParseFloat(Concentration, message);
		Concentration = (float)message.args[0];
	}
}
